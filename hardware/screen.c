 ...
 * @brief   串口屏控制模块 —— 核心实现
 *
 * 本文件实现了 STM32F407 环境保障系统与串口屏（UART5, 512000bps）之间的
 * 完整通信控制逻辑，涵盖以下子系统：
 *
 * 【架构概述】
 *   1. 协议编解码层
...
/* ======================== 头文件包含 ======================== */
#include "uart4_screen.h"       /* UART5 串口屏底层驱动 */
...
static SemaphoreHandle_t s_screen_tx_mutex = NULL;         /**< UART5 发送互斥锁，保护多任务并发发送 */
...
/**
 * @brief   获取继电器连接状态（复用 MQTT 模块缓存）
 * @return  1=继电器已连接, 0=未连接
 *
 * 直接复用 MQTT 模块维护的继电器链路缓存，避免屏幕任务重复访问 Modbus 总线，
 * 也让屏幕与 MQTT/状态上报使用同一套去抖后的在线判定。
 */
static uint8_t Screen_GetRelayConnectedCached(void)
{
    return MqttApp_GetRelayLinkCached();
}
 ...
 *
 * 无限循环中执行以下操作：
 *   1. 从 UART5 接收缓冲区读取所有待处理字节，逐字节解析帧
 *   2. 更新本地时钟
